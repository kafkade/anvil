//! State management module for Winforge
//!
...
/// Get the Winforge data directory (~/.winforge)
pub fn get_winforge_dir() -> Result<PathBuf> {
    let home =
        dirs::home_dir().ok_or_else(|| anyhow::anyhow!("Could not determine home directory"))?;
    let winforge_dir = home.join(".winforge");

    // Create directory if it doesn't exist
    if !winforge_dir.exists() {
        std::fs::create_dir_all(&winforge_dir)?;
    }

    Ok(winforge_dir)
}

/// Get the state directory (~/.winforge/state)
pub fn get_state_dir() -> Result<PathBuf> {
    let state_dir = get_winforge_dir()?.join("state");
...
/// Get the cache directory (~/.winforge/cache)
pub fn get_cache_dir() -> Result<PathBuf> {
    let cache_dir = get_winforge_dir()?.join("cache");
