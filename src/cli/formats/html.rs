...
impl HtmlFormatter {
    ...
    fn write_header(&self, writer: &mut dyn Write, title: &str, subtitle: &str) -> Result<()> {
        ...
        writeln!(writer, "    <meta name=\"generator\" content=\"Winforge {}\">" , env!("CARGO_PKG_VERSION"))?;
        ...
        writeln!(
            writer,
            "    <p class=\"meta\">Generated by Winforge {} on {}</p>",
    ...
    fn write_footer(&self, writer: &mut dyn Write) -> Result<()> {
        ...
        writeln!(
            writer,
            "    <p>Generated by <strong>Winforge</strong> v{} &middot; {}</p>",
            env!("CARGO_PKG_VERSION"),
            Utc::now().format("%Y-%m-%d %H:%M:%S UTC")
        )?;
        writeln!(writer, "</footer>")?;
...
impl OutputFormatter for HtmlFormatter {
    fn format_list(&self, workloads: &[WorkloadInfo], writer: &mut dyn Write) -> Result<()> {
        let title = self.title.as_deref().unwrap_or("Winforge Workloads");
    ...
    fn format_health(&self, report: &HealthReport, writer: &mut dyn Write) -> Result<()> {
        let title = self
            .title
            .as_deref()
            .unwrap_or("Winforge Health Check Report");
    ...
    fn format_install(&self, summary: &InstallSummary, writer: &mut dyn Write) -> Result<()> {
        let title = self.title.as_deref().unwrap_or("Winforge Installation Report");
    ...
    fn format_validate(&self, results: &ValidationResults, writer: &mut dyn Write) -> Result<()> {
        let title = self.title.as_deref().unwrap_or("Winforge Validation Report");
