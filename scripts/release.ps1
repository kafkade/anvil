...
<#
.SYNOPSIS
    Prepare and cut an Anvil release.
.DESCRIPTION
...
$prevVersion = [regex]::Match($changelog, '\[(\d+\.\d+\.\d+)\]:\s*https://').Groups[1].Value
$newLinks = "[Unreleased]: https://github.com/kafkade/anvil/compare/v$Version...HEAD`n[$Version]: https://github.com/kafkade/anvil/compare/v$prevVersion...v$Version"
$changelog = $changelog -replace '\[Unreleased\]:\s*https://github.com/kafkade/anvil/compare/v[\d.]+\.\.\.HEAD', $newLinks
...
if ($Push) {
    ...
    Write-Host "  ✓ Pushed — release workflow will start automatically" -ForegroundColor Green
} else {
